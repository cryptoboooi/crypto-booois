...
module.exports = () => {
      ...
      acc[level] = {
        get () {
          ...
          return Object.defineProperty(
            byLevel.map((l) => [l.title, l.message]),
            'byTitle',
            {
              value: (title) => byLevel
  ...
  const logStrings = Object.defineProperties(
    [],
    LEVELS.reduce((acc, level) => {
      acc[level] = {
        get () {
    ...
    stderr: {
      write: (str) => {
        ...
        if (logMatch) {
          ...
          logStrings.push(fullMessage)
        } else {
          outputErrors.push([str])
          outputErrorStrings.push(str)
        }
      },
    },
    stdout: {
  ...
  return {
    ...
    logs: {
      outputs,
      outputStrings,
      joinedOutput: () => outputStrings.join('\n').trimEnd(),
      clearOutput: () => {
      ...
      outputErrors,
      outputErrorStrings,
      joinedOutputError: () => outputErrorStrings.join('\n').trimEnd(),
      logs,
      logStrings,
      joinedLogs: () => RAW_LOGS.map((l) => l.join(' ')).join('\n').trimEnd(),
      clearLogs: () => {
        RAW_LOGS.length = 0
        logs.length = 0
        logStrings.length = 0
