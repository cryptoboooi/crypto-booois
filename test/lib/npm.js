...
t.test('npm.load', async t => {
  ...
  await t.test('basic loading', async t => {
    const { npm, logStrings: logs, prefix: dir, cache, other } = await loadMockNpm(t, {
  ...
  await t.test('forceful loading', async t => {
    const { logStrings: logs } = await loadMockNpm(t, {
      globals: {
  ...
  await t.test('node is a symlink', async t => {
    ...
    const { Npm, npm, logStrings: logs, outputStrings: outputs, prefix } = await loadMockNpm(t, {
      prefixDir: {
  ...
  await t.test('workspace-aware configs and commands', async t => {
    const { npm, outputs, joinedOutput } = await loadMockNpm(t, {
      prefixDir: {
        packages: {
          a: {
            'package.json': JSON.stringify({
